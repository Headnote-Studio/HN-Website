...
- **Four-pillar order (changed 2026-07-10, James's decision):** the pillars are **Visibility → Reviews → Content → Measurement**. The **Referrer network is NOT a pillar and is NOT a service** — per D39 (14 Jul) Referrer Activation was dropped from the ladder; only the referrer *map* survives, as a finding inside the Diagnostic ("Your local network of influence, mapped" band on Services). Never offer introductions, outreach or activation. Move each pillar's icon, heading and text together as a block.
